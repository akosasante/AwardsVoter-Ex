defmodule AwardsVoter.Web.AdminShowEdit do
  ...
  def handle_event("save_show_details", %{"show" => show}, %{assigns: %{updated_show: updated_show}} = socket) do
    {:ok, new_show} = Show.update(updated_show, show)
    socket = assign(socket, :updated_show, new_show)
    {:noreply, socket}
  end

  def handle_event("save_category_details", %{"category" => updated_category_map}, %{assigns: %{selected_category_name: selected_category_name, updated_show: updated_show}} = socket) do
  ...
  def handle_event("save_contestant_details", %{"contestant" => updated_contestant_map}, %{assigns: %{selected_contestant_name: selected_contestant_name, selected_category_name: selected_category_name, updated_show: updated_show}} = socket) do
    original_contestant = Admin.get_contestant_by_name(updated_show, selected_category_name, selected_contestant_name)
    new_show = Admin.update_show_contestant(updated_show, selected_category_name, original_contestant, updated_contestant_map)
    socket =
      socket
      |> assign(:updated_show, new_show)
  ...
  def handle_event("show_contestant", %{"contestant" => contestant_name}, socket) do
    socket =
      if socket.assigns.show_contestant and socket.assigns.selected_contestant_name == contestant_name do
        socket
  ...
  def handle_event("delete_contestant", %{"contestant" => contestant_name}, %{assigns: %{updated_show: show, selected_category_name: selected_category_name}} = socket) do
