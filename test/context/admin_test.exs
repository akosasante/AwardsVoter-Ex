defmodule AwardsVoter.Context.AdminTest do
  ...
  defmodule MockShows do
    def get(show_name) do
      send(self(), :get_show)
      assert show_name == test_show().name
      test_show()
    end
    def put(_show_tuples) do
  ...
  defmodule MockShowsAddCategory do
    def get(show_name) do
      send(self(), :get_show)
      assert show_name == test_show().name
      %{test_show() | categories: []}
    end
    def put(show_tuples), do: MockShows.put(show_tuples)
  end
  ...
  defmodule MockVoter do
    def save_ballot(_ballot, show_name) do
      assert show_name == test_show().name
      send(self(), :update_ballots)
      :ok
    end

    def list_ballots_for_show(_show_name), do: [test_ballot()]
  end

  setup do
    Application.put_env(:awards_voter, :show_manager_mod, MockShows)
  ...
  describe "get_category_from_show/2" do
    ...
    test "returns :show_not_found if show name is invalid" do
      Application.put_env(:awards_voter, :show_manager_mod, MockInvalidShows)
  ...
  describe "add_category_to_show/2" do
    test "returns success tuple upon adding category and saves result to both tables" do
      Application.put_env(:awards_voter, :show_manager_mod, MockShowsAddCategory)
  ...
  describe "add_contestant_to_show_category/3" do
    test "returns success tuple upon adding contestant and saves result to show and ballot tables" do
      Application.put_env(:awards_voter, :show_manager_mod, MockShowsAddContestant)
