defmodule AwardsVoter.Context.Voting.Ballots.BallotTest do
  ...
  describe "Ballot.save_ballot/3" do
    test "should return successful tuple if save was successful" do
      ...
      ballot = test_ballot()
      assert {:ok, ^ballot} = Ballot.save_ballot(ballot, test_show().name, VoterSaveBallot)
    end
    ...
    test "should return :error_saving if the save was unsuccessful" do
      ...
      assert :error_saving = Ballot.save_ballot(test_ballot(), "Invalid Show Name", InvalidVoterSaveBallot)
    end
  end
  
  describe "Ballot.get_ballot_by_voter_and_show/3" do
    test "should return :not found if no ballot for voter found" do
      ...
      assert :not_found = Ballot.get_ballot_by_voter_and_show("Invalid Voter Name", test_show().name, NotFoundVoterGet)
    end
    ...
    test "should return success tuple if ballot found" do
      ...
      ballot = test_ballot()
      assert {:ok, ^ballot} = Ballot.get_ballot_by_voter_and_show(test_ballot().voter, test_show().name, VoterModule)
    end
    ...
    test "should return :error_finding for all other errors" do
      ...
      assert :error_finding = Ballot.get_ballot_by_voter_and_show(test_ballot().voter, test_show().name, ErrorVoterModule)
    end
  end
end
